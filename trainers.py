...
class Trainer:
    ...
    def bpr_loss(self, seq_out, pos_ids, neg_ids):
        ...
        pos_logits = torch.sum(pos * seq_emb, dim=-1)  # [B*L]
        neg_logits = torch.sum(neg * seq_emb, dim=-1)  # [B*L]

        # valid target mask (padding 제거)
        istarget = (
            (pos_ids > 0)
            .view(pos_ids.size(0) * self.model.args.max_seq_length)
            .float()
        )  # [B*L]

        # BPR loss
        loss = -torch.log(
            torch.sigmoid(pos_logits - neg_logits) + 1e-24
        ) * istarget

        return torch.sum(loss) / torch.sum(istarget)
    ...
    def save_item_embeddings(self, file_name):
        item_embeddings = self.model.item_embeddings.weight.cpu().data.numpy()
        np.save(file_name, item_embeddings)
